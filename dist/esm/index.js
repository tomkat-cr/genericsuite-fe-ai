import React, { useState, useEffect, useContext, createContext, useRef, useReducer } from 'react';
...
import { faGreaterThan, faPlus, faEye, faEdit, faTrashAlt, faCheck, faList, faMicrophone, faStop, faArrowUp, faTimes, faPaperclip, faCamera, faCameraRetro, faExchangeAlt, faTrash, faLessThan, faPlay, faArrowDown } from '@fortawesome/fontawesome-free-solid';
...
function login(username, password) {
  ...
  const requestOptions = {
    method: 'POST',
    headers: {
      "Authorization": "Basic " + Buffer$1.from(username + ":" + password).toString('base64')
    }
  };
...
const HomePage = _ref => {
  ...
  const [currentUser, setCurrentUser] = useState(authenticationService$1.currentUserValue);
  useEffect(() => {
